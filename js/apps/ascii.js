  ...
  setTimeout(() => {
    ...
    document.getElementById('asc-snap').onclick = () => {
      ...
      try {
        ...
        const name = 'ascii-cam-' + new Date().toISOString().slice(0,19).replace(/[:T]/g,'-') + '.jpg';
        photos.unshift({ data: dataURL, name, date: new Date().toLocaleDateString() });
        // Keep max 100 photos to avoid quota
        if (photos.length > 100) photos = photos.slice(0, 100);
        localStorage.setItem(GALLERY_KEY, JSON.stringify(photos));
        // Show saved toast
        showToast98 && showToast98('Saved!', 'Photo saved to Gallery.', '📷');
      } catch(e) {
        // If localStorage quota exceeded, try smaller quality
        try {
          const small = ascCV.toDataURL('image/jpeg', 0.5);
          let photos = [];
          try { photos = JSON.parse(localStorage.getItem(GALLERY_KEY) || '[]'); } catch(e2) {}
          photos.unshift({ data: small, name: 'ascii-cam-' + Date.now() + '.jpg', date: new Date().toLocaleDateString() });
          if (photos.length > 50) photos = photos.slice(0, 50);
          localStorage.setItem(GALLERY_KEY, JSON.stringify(photos));
          showToast98 && showToast98('Saved!', 'Photo saved to Gallery.', '📷');
        } catch(e2) {
          showToast98 && showToast98('Error', 'Storage full. Delete some gallery photos.', '⚠️');
        }
      }
    };
