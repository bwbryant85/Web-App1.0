...
function initMessages98() {
  ...
  function buildMain() {
    ...
    listSb.style.cssText = 'display:flex;align-items:center;justify-content:space-between;gap:6px;padding:2px 6px;';
    listSb.innerHTML = `<div class="win-status-pane" style="flex:1;">Logged in as @${me}</div>
      <button id="msg-sim-btn" class="btn98" style="font-size:.72rem;padding:2px 7px;white-space:nowrap;">🤖 Simulator</button>`;
    listPanel.appendChild(listSb);

    // Open simulator panel when clicked
    ...
    /* ── Simulator: local fake-bot chat for testing ── */
    function openSimulator() {
      if (unsubChat) { unsubChat(); unsubChat = null; }
      chatPanel.innerHTML = '';

      const simBotName = 'simulator-bot';
      const botReplies = [
        ...
        "Can you say that again? Didn't catch it",
        'haha yeah for sure',
        ...
        'k',
        'YES exactly!!',
        "That's actually wild when you think about it",
        'Sending ✓✓',
        "ok so basically what I'm saying is... idk actually",
      ];

      const simHead = document.createElement('div');
      simHead.style.cssText = 'flex-shrink:0;display:flex;align-items:center;gap:8px;padding:4px 8px;background:var(--win-title-active);';
      simHead.innerHTML = `
        <button id="sim-back" class="btn98" style="padding:2px 8px;font-size:.82rem;">◄ Back</button>
        <span style="font-family:var(--pixel-font);font-size:.9rem;color:#fff;flex:1;">🤖 Simulator <span style="font-size:.7rem;opacity:.7;">(local — no server)</span></span>
      `;
      chatPanel.appendChild(simHead);

      const simArea = document.createElement('div');
      simArea.style.cssText = 'flex:1;overflow-y:auto;-webkit-overflow-scrolling:touch;padding:8px;display:flex;flex-direction:column;gap:4px;background:#fff;';
      chatPanel.appendChild(simArea);

      const typingEl = document.createElement('div');
      typingEl.style.cssText = 'flex-shrink:0;font-family:var(--pixel-font);font-size:.75rem;color:#808080;padding:2px 8px;min-height:1.3em;background:#fff;border-top:1px solid var(--win-chrome-dark);';
      chatPanel.appendChild(typingEl);

      const simInputRow = document.createElement('div');
      simInputRow.style.cssText = 'flex-shrink:0;display:flex;gap:6px;padding:5px 8px;border-top:2px solid var(--win-chrome-dark);background:var(--win-chrome);';
      simInputRow.innerHTML = `
        <input id="sim-inp" type="text" maxlength="500" placeholder="Type to test messaging…"
          style="flex:1;padding:4px 8px;font-family:var(--pixel-font);font-size:.88rem;background:#fff;border:2px solid;border-color:var(--win-chrome-dark) var(--win-chrome-light) var(--win-chrome-light) var(--win-chrome-dark);color:var(--win-text);outline:none;">
        <button id="sim-send" class="btn98 primary" style="padding:4px 12px;">Send</button>
      `;
      chatPanel.appendChild(simInputRow);
      chatPanel.style.transform = 'translateX(0)';

      function addSimBubble(text, fromMe) {
        const row = document.createElement('div');
      ...
      addSimBubble("Hey! I'm the simulator bot 🤖 Send me anything to test the chat UI!", false);

      const simInp  = simInputRow.querySelector('#sim-inp');
      const simSend = simInputRow.querySelector('#sim-send');
      let botTimer = null;

      function doSimSend() {
        const text = simInp.value.trim();
        if (!text) return;
        simInp.value = '';
        addSimBubble(text, true);
        simSend.disabled = true;
        const thinkMs = 700 + Math.random() * 1800;
        typingEl.textContent = 'simulator-bot is typing…';
        botTimer = setTimeout(() => {
          typingEl.textContent = '';
          addSimBubble(botReplies[Math.floor(Math.random() * botReplies.length)], false);
          simSend.disabled = false;
          simInp.focus();
        }, thinkMs);
      }

      simSend.addEventListener('click', doSimSend);
      simInp.addEventListener('keydown', e => { if (e.key === 'Enter') doSimSend(); });
      simInp.focus();

      simHead.querySelector('#sim-back').addEventListener('click', () => {
