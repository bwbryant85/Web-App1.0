/* ════════════════════════════════════════════════════════════
   MAPS v2 — Win98 Edition
   Leaflet + OpenStreetMap | Nominatim geocoding | No API key
   TRUE pixel downscale • Chunky 98 chrome • Live suggestions
   ════════════════════════════════════════════════════════════ */
...
function initMaps98() {
  ...
  const searchBtn  = makeBtn98('🔍 Find');
  ...
  /* Leaflet renders here at 1/PIXEL_SCALE size, then scaled up via CSS transform */
  ...
  function loadLeaflet(cb) {
    ...
    script.onerror = () => {
      loadPanel.innerHTML = '<div style="font-family:VT323,monospace;font-size:18px;color:#800000;">⚠️ Network Error</div><div style="font-family:VT323,monospace;font-size:13px;color:#000;">Could not load map library.</div>';
    };
  ...
  function initLeafletMap() {
    ...
    if (!document.getElementById(styleId)) {
      ...
      st.textContent = `
        ...
        #${leafletHost.id} .leaflet-popup-content-wrapper { background:#c0c0c0 !important; border:2px outset #fff !important; border-radius:0 !important; box-shadow:2px 2px 0 #000 !important; font-family:VT323,monospace; font-size:14px; padding:6px 10px !important; }
        #${leafletHost.id} .leaflet-popup-tip { background:#c0c0c0 !important; }
        #${leafletHost.id} .leaflet-control-zoom { display:none; }
        #${leafletHost.id} .leaflet-popup-close-button { color:#000080 !important; }
      `;
  ...
  function makeMarker98() {
    return L.divIcon({ className:'', html:`<div style="width:12px;height:12px;background:#000080;border:2px solid #fff;box-shadow:1px 1px 0 #000,-1px -1px 0 #000;position:relative;"><div style="position:absolute;bottom:-8px;left:3px;width:2px;height:8px;background:#000;"></div></div>`, iconSize:[12,20], iconAnchor:[6,20], popupAnchor:[0,-20] });
  }
  ...
  function placeMarker(lat, lng, label) {
    ...
    currentMarker = L.marker([lat,lng], { icon:makeMarker98() }).addTo(mapInstance).bindPopup(`<b>${label}</b>`).openPopup();
  ...
  function doSearch(query) {
    ...
    fetch(`https://nominatim.openstreetmap.org/search?q=${encodeURIComponent(query)}&format=json&limit=1`, { headers:{'Accept-Language':'en'} })
      .then(r => r.json())
      .then(data => {
        if (!data || !data.length) { statusPane.textContent = `No results for "${query}".`; showToast98&&showToast98('Location not found','error'); return; }
      ...
      .catch(() => { statusPane.textContent = 'Search failed. Check connection.'; showToast98&&showToast98('Search error','error'); });
  }
  ...
  function renderSuggestions(items) {
    ...
    items.forEach((item, i) => {
      ...
      row.addEventListener('mousedown', (e) => {
        ...
        placeMarker(la, ln, item.display_name.split(',').slice(0,2).join(','));
  ...
  function goToMyLocation() {
    if (!navigator.geolocation) { showToast98&&showToast98('Geolocation not supported','error'); return; }
    ...
    navigator.geolocation.getCurrentPosition(
      (pos) => {
        ...
        userMarker = L.marker([la,ln], { icon:makeUserMarker98() }).addTo(mapInstance).bindPopup('<b>📍 You are here</b>').openPopup();
        statusPane.textContent = `Location: ${la.toFixed(4)}, ${ln.toFixed(4)}`;
        locBtn.disabled = false;
      },
      () => { statusPane.textContent = 'Location access denied.'; showToast98&&showToast98('Location denied','error'); locBtn.disabled=false; },
      { timeout:8000 }
    );
  }
  ...
  pixBtn.addEventListener('click', () => {
    ...
    /* In pix mode: hide leaflet host (it renders at wrong scale visually) and show scanlines */
    /* In normal mode: show leaflet host normally and hide scanlines */
