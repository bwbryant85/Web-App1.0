...
function initClock() {
  ...
  const showDig = anim => {
    ...
    const build = () => {
      ...
      /* Digital time display — Win98 sunken inset style */
  ...
  const showAna = anim => {
    ...
    const build = () => {
      ...
      /* Canvas wrapped in Win98 inset border */
  ...
  const drawAna = sz => {
    ...
    const draw = () => {
      ...
      ctx.strokeStyle = 'rgba(0,255,65,.3)'; ctx.lineWidth = 1.5; ctx.stroke();
      ...
      for (let i = 0; i < 60; i++) {
        ...
        ctx.strokeStyle = maj ? 'rgba(0,255,65,.9)' : 'rgba(0,255,65,.25)';
      ...
      /* Hands */
      hand(ha, r * .55, sz * .024, '#00ff41');
      hand(ma, r * .76, sz * .016, '#00ff41');
      hand(sa, r * .84, sz * .011, '#ff4136');

      /* Center dot */
      ctx.beginPath(); ctx.arc(cx, cy, sz * .02, 0, Math.PI * 2);
      ctx.fillStyle = '#00ff41'; ctx.shadowColor = '#00ff41'; ctx.shadowBlur = 8; ctx.fill();
