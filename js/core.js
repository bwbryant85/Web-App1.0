...
let homeEditMode = false; // iOS-style jiggle edit mode (modern theme only)
...
function enterHomeEditMode() {
  ...
  homeEditMode = true;
  /* First play the press-down-then-spring animation */
  document.body.classList.add('home-editing-enter');
  haptic && haptic('medium');
  /* After enter animation (~420ms), switch to continuous jiggle */
  setTimeout(() => {
...
const homeDragState = {
  ...
  startX: 0,
  startY: 0,
  offsetX: 0,  /* pointer offset from clone top-left */
  offsetY: 0,
};
...
function beginHomeDrag(e, icon, pageIdx) {
  ...
  /* Store finger-to-icon-corner offset so drag tracks finger exactly */
  ...
  clone.style.opacity = isModernTheme() ? '0.95' : '0.92';
  clone.style.transform = isModernTheme() ? 'scale(1.12) rotate(2deg)' : 'scale(1.05)';
  clone.style.transition = isModernTheme() ? 'transform 0.15s cubic-bezier(0.34,1.56,0.64,1)' : '';
  clone.style.zIndex = '9998';
  clone.style.willChange = 'left, top';
  /* Remove the ::before badge from clone to avoid double badges */
    ...
    showDialog98(isModern ? 'Shut down' : 'Shut Down',
      ...
      {label: isModern ? 'Shut down' : 'Shut Down', primary:true, action:() => {
        if (isModern) {
          ...
          // Win11 logo (four coloured squares)
          ov.innerHTML = `
            <svg width="72" height="72" viewBox="0 0 88 88" fill="none" xmlns="http://www.w3.org/2000/svg">
              <rect x="0"  y="0"  width="40" height="40" rx="3" fill="#F25022"/>
              <rect x="48" y="0"  width="40" height="40" rx="3" fill="#7FBA00"/>
            ...
            <div style="display:flex;flex-direction:column;align-items:center;gap:28px;">
              <div style="font-family:'Inter',system-ui,sans-serif;font-size:1.1rem;color:rgba(255,255,255,.85);letter-spacing:.02em;" id="sd11-txt">Shutting down</div>
          ...
          setTimeout(() => {
            const txt = ov.querySelector('#sd11-txt');
            if (txt) txt.textContent = 'See you later';
            setTimeout(() => {
              ov.style.transition = 'opacity 1.2s ease';
              ov.style.opacity = '0';
              setTimeout(() => {
                document.body.innerHTML = '<div style="background:#000;width:100%;height:100%;position:fixed;inset:0;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:32px;"><svg width="56" height="56" viewBox="0 0 88 88" fill="none"><rect x="0" y="0" width="40" height="40" rx="3" fill="#F25022"/><rect x="48" y="0" width="40" height="40" rx="3" fill="#7FBA00"/><rect x="0" y="48" width="40" height="40" rx="3" fill="#00A4EF"/><rect x="48" y="48" width="40" height="40" rx="3" fill="#FFB900"/></svg><div style="font-family:Inter,system-ui,sans-serif;font-size:1rem;color:rgba(255,255,255,.6);letter-spacing:.02em;">You can close this tab</div></div>';
              }, 1200);
            }, 2200);
          }, 2000);
  ...
  APP_PAGES.forEach((apps, pageIdx) => {
    ...
    orderedApps.forEach(app => {
      ...
      icon.addEventListener('click', () => {
        if (homeDragState.dragging) return;
        if (homeEditMode) return; /* don't open app while jiggling */
        haptic('light');
        OS.openApp(app.id);
      });
      icon.addEventListener('pointerdown', (e) => {
        if (e.pointerType === 'mouse' && e.button !== 0) return;
        /* Check if tapping the × badge (top-left 28x28 area of icon) */
        if (homeEditMode && isModernTheme()) {
          const iconRect = icon.getBoundingClientRect();
          const bx = e.clientX - iconRect.left;
          const by = e.clientY - iconRect.top;
          if (bx <= 28 && by <= 28) {
            e.stopPropagation();
            e.preventDefault();
            /* Uninstall / hide the app */
            const appId = icon.dataset.appId;
            if (appId) {
              let installed = JSON.parse(localStorage.getItem('ipocket_installed') || 'null');
              if (!installed) installed = window.APP_REGISTRY ? window.APP_REGISTRY.map(a=>a.id) : [];
              installed = installed.filter(id => id !== appId);
              localStorage.setItem('ipocket_installed', JSON.stringify(installed));
              buildGrid();
            }
            return;
          }
