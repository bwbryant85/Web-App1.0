...
/* ── WIN11 ORB BACKGROUND helper (shared by startup overlay & shutdown) ── */
function createOrbCanvas() {
  ...
  function draw() {
    ...
    orbs.forEach(o => {
      ...
      if (o.y < -0.3) o.y = 1.3;
      if (o.y > 1.3)  o.y = -0.3;
      const pulse = 0.85 + 0.15 * Math.sin(t * 0.6 + o.phase);
      const rx = o.x * W, ry = o.y * H, rr = Math.min(W, H) * o.r * pulse;
      const grad = ctx.createRadialGradient(rx, ry, 0, rx, ry, rr);
      grad.addColorStop(0,   `hsla(${o.hue},${o.sat}%,62%,0.38)`);
      grad.addColorStop(0.4, `hsla(${o.hue},${o.sat}%,50%,0.22)`);
      grad.addColorStop(0.75,`hsla(${o.hue},${o.sat}%,40%,0.09)`);
      grad.addColorStop(1,   `hsla(${o.hue},${o.sat}%,30%,0)`);
      ctx.fillStyle = grad;
      ctx.fillRect(0, 0, W, H);
    });
    // blur via multi-pass low-alpha overlay
