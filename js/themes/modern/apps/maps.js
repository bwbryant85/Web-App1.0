/* ════════════════════════════════════════════════════════════
   MAPS v3.3 — Modern (Windows 11 / Apple Maps style)
   MapLibre GL JS + OpenFreeMap liberty | Nominatim
   Satellite = Esri raster injected ABOVE road tiles,
   road labels remain visible on top (hybrid look).
   3D buildings work in both map + satellite mode.
   ════════════════════════════════════════════════════════════ */
...
function initMaps98() {
  ...
  /* ══════════════════════════════════════════════════════════
     SATELLITE STRATEGY:
     Inject Esri raster as a layer BETWEEN the road background
     and the road fill layers — so roads/labels still show on
     top of satellite (hybrid look, same as Apple Maps).
     Toggle visibility only. NEVER call setStyle() again.
  ══════════════════════════════════════════════════════════ */
  function injectSatLayer() {
    ...
    try {
      ...
      /* Find first non-background layer to insert before */
      const layers = mapInstance.getStyle().layers;
      let insertBefore = null;
      for (const l of layers) {
        if (l.type !== 'background') { insertBefore = l.id; break; }
      }

      mapInstance.addLayer(
        { id:'sat-imagery', type:'raster', source:'esri-sat',
  ...
  function setMode(mode) {
    ...
    if (mode==='satellite') {
      setSat(true);
      /* If 3D is already on, show buildings now that satellite is active */
      if (is3D) ensure3D();
      showStatus('Satellite',1500);
    } else {
      setSat(false);
      /* Remove buildings when leaving satellite regardless of 3D state */
      remove3D();
      showStatus('Map',1500);
    }
  }
  ...
  function toggle3D() {
    ...
    if (is3D) {
      ...
      /* Buildings only appear in satellite+3D combo */
